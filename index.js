...
const ONPREM_BASE_URL =
  process.env.ONPREM_BASE_URL || "http://35.209.18.19:8080";
const GCP_BASE_URL = process.env.GCP_BASE_URL || "http://34.42.37.99";
...
app.all("*", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const onPremCheck = await axios.post(`${ONPREM_BASE_URL}/checkuser`, {
        email,
      });
      ...
      if (onPremCheck.data.success && onPremCheck.data.exists) {
        ...
        const gcpCheck = await axios.post(`${GCP_BASE_URL}/checkuser`, {
          email,
        });
